...
function sendUnifile(unifile, res, session, service, path) {
  return Promise.all([
  ...
  constructor (options) {
    ...
    this.get(/\/(.*)\/icon\/(.*)/, withCache, (req, res) => {
      ...
      if(['jpg', 'jpeg', 'png'].includes(ext)) {
        sendUnifile(this.unifile, res, req.session.unifile, req.params[0], req.params[1])
        .then(([fileInfo, fileContent]) => {
          if(fileInfo.mime) res.type(fileInfo.mime);
          const {width, height} = sizeOf(fileContent);
          if(options.enableImageMagick && (width > THUMBNAIL_SIZE || height > THUMBNAIL_SIZE)) {
