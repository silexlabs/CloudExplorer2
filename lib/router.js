...
const sharp = (() => {
  try {
    ...
    console.warn('Warning: could not load sharp, thumbnails will be deactivated. ', e);
    return null;
  }
})()
...
// Build a session from headers
// This allows to use ftp/sftp/etc. connectors onver http
function sessionFromHeaders(headers) {
  // Dirty fix for @lexoyo
  if(headers['UNIFILE_FTP_PASSWORD']) {
    headers['UNIFILE_FTP_TOKEN'] = headers['UNIFILE_FTP_PASSWORD']
    headers['UNIFILE_SFTP_TOKEN'] = headers['UNIFILE_FTP_PASSWORD']
  }
  // Build session from headers
  return Object.entries(headers)
    .map(([key, value]) => ([key.toLowerCase(), value]))
    .filter(([key, value]) => key.startsWith('unifile_'))
    .reduce((result, [key, value]) => {
      const [_, connector, ...cookieName] = key.split('_')
      return {
        ...result,
        [connector]: {
          ...result[connector],
...
/*
 * ///////////
 * Methods added to the Router class (because express make it a clone of a class)
 * ///////////
 */
/*
 * Add a custom service to unifile
 */
function addService (unifileConnector) {
  this.unifile.use(unifileConnector);
}

/*
 * Add an image bank
 * @bank {name, displayName, search, random}
 */
function addImageBank (bank) {
  imageBank.add(bank);
}
...
/**
 * The router class
 * This is not a real class, it can not take any methods
...
module.exports = class Router extends express.Router {
  ...
  constructor (options) {
    ...
    this.addImageBank = addImageBank;
    ...
    // Beaker and hyperdrive
    if (options.hyperdrive) {
      this.enableHyperdrive = true;
    }

    // Image banks
    this.use(imageBank.getRouter());
    if (options.unsplash && options.unsplash.accessKey) {
    ...
    this.use((req) => {
      req.session.unifile = {
        ...
        ...req.session.unifile,
        // Enable session from headers feature
        ...sessionFromHeaders(req.headers),
      };
      if(process.env.NODE_ENV === 'development') console.log('req.headers', req.headers, 'req.session.unifile', req.session.unifile);
      req.next();
    });
    ...
    this.get(/\/(.*)\/icon\/(.*)/, withCache, (req, res) => {
      ...
      .shift()
      .toLowerCase();
      if (sharp && options.thumbnails && options.thumbnails.extensions.includes(ext)) {
