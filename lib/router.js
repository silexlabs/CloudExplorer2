...
module.exports = class Router extends express.Router {
  ...
  constructor (options) {
    ...
    this.use('/cloud-explorer', serveStatic(Path.resolve(__dirname, '../dist')));
    ...
    this.get('/:connector/oauth_callback', (req, res) => {
      if ('error' in req.query) {
        ...
        .catch((err) => {
          res.cookie('unifile_error', JSON.stringify(err));
          res.redirect('/cloud-explorer/handle-errors');
        });
      }
    });

    this.get('/cloud-explorer/handle-errors', (req, res) => {
      res.sendFile(Path.join(__dirname, '..', 'dist/handle-errors.html'));
    });
